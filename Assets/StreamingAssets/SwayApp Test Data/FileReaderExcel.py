...
for current_file in txtfiles:
    ...
    #data = pd.DataFrame({"Time":timeList[0]})
    ...
    sheet1.write(0,2,"Distance(d)")
    sheet1.write(0,3,"Abs Distance")
    sheet1.write(0,4,"Velocity")
    
    for i in range(len(timeList[0])):
        if i == 0:
            sheet1.write(i+1,0,str(timeList[0][i]))
            sheet1.write(i+1,1,str(round(filteredList[0][i][6], 4)))
        else:
            sheet1.write(i+1,0,str(timeList[0][i]))
            sheet1.write(i+1,1,str(round(filteredList[0][i][6], 4)))
            sheet1.write(i+1,2,str(round(displacementX[i-1], 4)))
            sheet1.write(i+1,3,str(round(asbDisplacementX[i-1], 4)))
            sheet1.write(i+1,4,str(round(velocityX[i-1], 4)))

    sheet1.write(0,6,"Max Range") 
    sheet1.write(1,6,str(max(asbDisplacementX)))
    sheet1.write(0,7,"Mean Range")
    sheet1.write(1,7,str(statistics.mean(asbDisplacementX)))
    sheet1.write(0,8,"Peak Velocity")
    sheet1.write(1,8,str(max(velocityX)))
    sheet1.write(0,9,"Mean Velocity")
    sheet1.write(1,9,str(statistics.mean(velocityX)))

    sheet2 = wb.add_sheet('Y')
    sheet2.write(0,0,"Time") 
    sheet2.write(0,1,"CoPx")
    sheet2.write(0,2,"Distance(d)")
    sheet2.write(0,3,"Abs Distance")
    sheet2.write(0,4,"Velocity")
    
    for i in range(len(timeList[0])):
        if i == 0:
            sheet2.write(i+1,0,str(timeList[0][i]))
            sheet2.write(i+1,1,str(round(filteredList[0][i][7], 4)))
        else:
            sheet2.write(i+1,0,str(timeList[0][i]))
            sheet2.write(i+1,1,str(round(filteredList[0][i][7], 4)))
            sheet2.write(i+1,2,str(round(displacementY[i-1], 4)))
            sheet2.write(i+1,3,str(round(asbDisplacementY[i-1], 4)))
            sheet2.write(i+1,4,str(round(velocityY[i-1], 4)))

    sheet2.write(0,6,"Max Range") 
    sheet2.write(1,6,str(max(displacementY)))
    sheet2.write(0,7,"Mean Range")
    sheet2.write(1,7,str(statistics.mean(displacementY)))
    sheet2.write(0,8,"Peak Velocity")
    sheet2.write(1,8,str(max(velocityY)))
    sheet2.write(0,9,"Mean Velocity")
    sheet2.write(1,9,str(statistics.mean(velocityY)))
